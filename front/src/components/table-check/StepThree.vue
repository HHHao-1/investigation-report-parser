        ...
        >
          ...
          >
            <template #header="scope" v-if="editable">
              <dropdown ref="close11" :scope="scope" :text="scope.column.label" :is-first="true" :table="1"
                        @tableDate="getDate"
              />
            </template>
          </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      thClass: {},
      must: {one: [1, 2, 3], two: [1, 2, 3]},
      tableData1: [],
      tableData2: [],
      sheetDto1: {
        pageName: this.text + '-充币方向',
        map: []
      },
      sheetDto2: {
        pageName: this.text + '-提币方向',
        map: []
      },
    }
  },
  ...
  mounted() {
    ...
    if (!this.editable && (sessionStorage.getItem('checkData') || 'e') !== 'e') {
      this.tableData1 = JSON.parse(sessionStorage.getItem('checkData')).depositList
      this.tableData2 = JSON.parse(sessionStorage.getItem('checkData')).withdrawList
    }
  },
  methods: {
    format(row, column) {
